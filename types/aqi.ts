...
export const getAlertMessage = (aqi: number): AlertMessage | null => {
  ...
  if (aqi <= 100) return { id: Date.now().toString(), type: 'warning', title: 'Moderate Air Quality', message: 'Consider reducing outdoor activities.' };
  if (aqi <= 150) return { id: Date.now().toString(), type: 'danger', title: 'Unhealthy Air Quality', message: 'Wear a mask and stay indoors.' };
  return { id: Date.now().toString(), type: 'danger', title: 'Hazardous Air Quality', message: 'Stay indoors immediately!' };
};